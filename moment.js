...
function toNumber(value) {
    // Handle cases where value is already a number
    ...
    if (typeof value === "string") {
        ...
        // Ensure that the parsed value is a finite number
        if (!isNaN(parsed) && isFinite(parsed)) {
            return parsed;
        }

        // Return NaN if the string can't be converted to a number
        return NaN;
    }

    // Handle cases where value is a boolean
    if (typeof value === "boolean") {
        return value ? 1 : 0;
    }

    // Handle cases where value is an object with a valueOf method
    if (value != null && typeof value.valueOf === "function") {
        var objValue = value.valueOf();
        if (typeof objValue === "number") {
            return objValue;
        }
    }

    // Default case: return NaN if the value can't be converted to a number
    return NaN;
}
